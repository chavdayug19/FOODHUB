...
router.get('/', auth, authorize('vendor', 'staff', 'admin'), orderController.getOrders);
router.get('/:id', orderController.getOrderById); // Public for tracking (or secured if using session)
router.put('/:orderId/vendor/:vendorId/status', auth, authorize('vendor', 'staff'), validate(updateStatusSchema), orderController.updateOrderStatus);
